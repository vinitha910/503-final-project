from planner_constrained_goal import AStar, TIMEOUT
...
def run_planner(env_parameters, render=None):
        resolution_m = 0.01
        #params = clamp_obs(env_parameters[:-4])
        ...
        env = Environment(100, 100, [[1,1,1,1], [1,1,1,1]])
        ...
        # Input x (m), y (m)
        if not (planner.set_start(env_parameters[0]/100., env_parameters[1]/100., env_parameters[2])):
            print(env_parameters[-4], ' ' , env_parameters[-5])
            success = False # no expansions, since initial config was invalid
        if not (planner.set_goal(env_parameters[3]/100., env_parameters[4]/100., env_parameters[5])):
            success = False # ditto
        ...
        if success:
            try:
                ...
                if success:
                    ...
                    error = True
                    print('constrained_goal_condition')
        ...
        print("Success:", success, "error:", error, "\tExpansions:", num_expansions, "\tTime:", planning_time)
        csv_writer.writerow([
            env_parameters,
            error,
            not not render,
...
if __name__ == "__main__":
    initial_mean = np.array([40, 70, pi/4, 70, 80, pi/4])
    initial_sigma = 2.0
